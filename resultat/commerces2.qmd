...
[^commerces2-2]: Merci à Théo Shayer, Ludovic Verre et Luc Garnier pour nous avoir partagé leur méthodologie.
...
[^commerces2-4]: Cette valeur se lit sur le tableau en prenant le coefficient du dernier décile, moins celui du premier (égal à 0 par référence) et en élevant $e$ à la puissance de cette différence.
...
$$
log(d_i) = a \times log(rev_{uc,i}) + b \times p_{enf,i} + c \times p_{act,i} + d_k \times \mathcal{A}_{i,k} + e + \varepsilon_i
$$
...
```{r}
#| label: fig-regresemc2
#| fig-cap: Modèles de régression sur les zones de l'EMC^2^
#| fig-asp: 1.2
...
resemc2$res |> 
  ...
  geom_pointrange(
    aes(x = dec, y = q, ymin = qmin, ymax = qmax, color = mod)) + 
  geom_point(
...
logit(Prob(courses)) = cte + décile\ surface\ équivalente + niveau\ de\ vie + couple + enfant + actif
$$
...
tbl_regression(MOD_freq, exponentiate = TRUE,
               label = list(se_dec = "Surface équivalente (déciles)",
                            couple = "En couple",
                            enfant = "Avec enfant(s)",
                            ACTOCCUP = "Actif occupé",
                            revenus_hab = "Niveau de vie (en euros)"),
               estimate_fun = label_style_ratio(digits = 4)) |>
  modify_header(label = "**Variable**", estimate = "**Coefficient**", conf.low = "**IC à 95%**", p.value = "**P**") |>
  modify_caption(caption = "**Régression logistique sur la probabilité de faire ses courses en voiture**")|> 
  modify_footnote(everything() ~ NA, abbreviation = TRUE)
    
```
Le modèle montre que la probabilité d'effectuer une course en voiture un jour donné diminue à mesure que l'offre de commerces (alimentaires) s'accroît. Une unité de surface équivalente en plus réduit la probabilité d'un facteur de 0,85, ce qui indique un effet assez prononcé sur les comportements. Le fait d'être en couple augmente la probabilité de faire des courses en voiture, tandis qu'avoir des enfants la réduit. L'un dans l'autre, les ménages avec enfants font moins fréquemment des courses en voiture, mais vraisemblablement rationalise plus fortement leurs déplacements et effectuent plus fréquemment des "grosses" courses en voiture. Les actifs ont également tendance à faire moins souvent des courses en voiture, probablement pour la même raison d'une organisation plus serrée et rationalisée des courses. Enfin, le niveau de vie produit aussi un effet : un ménage qui a un niveau de vie de 1000 € supérieur à un autre a une probabilité de faire des courses multipliée par 1,1. 
...
$$
log(distance) = cte + décile\ surface\ équivalente + niveau\ de\ vie + couple + enfant + actif + \epsilon
$$
...
tbl_regression(MOD_dist,
               ...
               estimate_fun = label_style_ratio(digits = 4)) |>
  modify_header(label = "**Variable**", estimate = "**Coefficient**", conf.low = "**IC à 95%**", p.value = "**P**") |> 
  modify_caption(caption = "**Régression sur la distance parcourue en voiture pour faire ses courses**") |> 
  modify_footnote(conf.low = NA, abbreviation = TRUE)

```

Le modèle montre que la distance parcourue en voiture pour faire des courses (ici alimentaires) est d'autant plus faible que l'on dispose de commerces de proximité. Pour chaque unité de surface équivalente supplémentaire dans son voisinage, un résident effectuera non seulement moins souvent ses courses en voiture, mais la distance parcourue sera réduite de 400m. Les personnes vivant en couple et celles ayant des enfants effectueront des trajets un peu plus long en voiture. Une explication possible est que plus la taille d'un ménage augmente, plus ses besoins et envies sont diversifiés et supposent de passer par une plus large gamme de magasins. Enfin, les actifs effectuent des trajets moins long, probablement parce qu'ils résident à proximité des zones denses et des zones d'emploi et, donc, ils vivent en général plus près des commerces que les inactifs.
