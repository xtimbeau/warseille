---
title: "Enquêtes sur la mobilité"
author:
  - name: Maxime Parodi
  - name: Xavier Timbeau 
---
...
Nous disposons de deux sources principales d'enquêtes sur les mobilités. La première est l'[enquête nationale sur la mobilité des personnes de 2019 (EMP 2019)](https://www.statistiques.developpement-durable.gouv.fr/resultats-detailles-de-lenquete-mobilite-des-personnes-de-2019). Publiée par le ministère de l'environnement, elle donne une information détaillée sur les déplacements d'un peu moins de 14 000 individus. On ne dispose pas d'information de localisation précises, puisque la représentativité spatiale est insuffisante. L'enquête a été réalisée entre 2018 et 2019. Une [enquête mobilité certifiée CEREMA (EMC^2^ AMP 2020) a été conduite sur le périmètre étendu de la métropole d'Aix-Marseille-Provence](https://www.agam.org/publication-des-chiffres-cles-de-lenquete-mobilite-emc%C2%B2/) en 2019 et 2020. et fournit un contrepoint intéressant à l'EMP. En effet, la représentativité spatiale est bien meilleure par construction et les 25 000 personnes interrogées permettent d'identifier les motifs de déplacement à une échelle fine, pratiquement jusqu'à l'IRIS. Par une coïncidence heureuse, les enquêtes ont été conduite sur deux périodes très proches , ce qui autorise les comparaisons.
...
L'EMC^2^ est opérée sur un périmètre spécifique, indiqué sur la carte de la \[-@fig-sub-emc2\]
...
Le @tbl-dens et la @fig-emp19 indiquent les kilomètres parcourus dans les deux enquêtes. Par ré-échantillonage, on estime les intervalles de confiance. Le @tbl-dens retranscrit pour les deux enquêtes ces éléments.

L'EMP, sur un champ plus large,  donne 30% de kilomètres totaux parcourus de moins que l'EMC^2^. Cette différence ne peut s'expliquer complètement par un biais d'échantillonage, les intervalles de confiance à 95% n'ayant pas d'intersection. L'EMP 2019 couvre plusieurs grandes agglomérations (pour les aires d'attractivité de plus de 700k habitants : Lyon, Toulouse, Bordeaux, Lille, ...) dont la géographie, la démographie et les comportements sont différents. Concernant les déplacements pour le trajet domicile travail, l'écart suivant les sources est inverse (30% de kilomètres en plus pour la métropole AMP).

Dans les deux enquêtes, la densité apparait comme plutôt négativement correllée aux distances parcourues. Dans l'EMC^2^, cependant, la mesure en 4 niveaux de densité n'est pas très précise. Dans le périmètre de la métropole, on observe en effet très peu d'individus dans les zones "peu denses" (27 observations) et "très peu denses" (aucune). Seule la différence entre "très dense" et "densité intermédiaire" a donc un sens, comme l'indiquent les intervalles de confiance à 95% du @tbl-dens. Pour l'EMP, en revanche, on dispose d'un peu plus d'observations sur ces zones peu denses, ce qui confirme la différence géographique entre les métropoles considérées.
...
enq$tab |> 
  ...
  tab_source_note(md(
    "*Note* : km parcourus par an en voiture, entre crochets : intervalle de confiance à 95%<br>
...
datag <- bind_rows(empg, emc2g) |> 
  mutate(src = factor(src, c("emp", "emc2"), c("EMP 2019", "EMC{.sup 2} AMP 2020") ))
