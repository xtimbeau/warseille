...
download_this(
  ...
  output_name = "co2_idINS_pdvR_MetAMP"
)
```
...
download_this(
  ...
  output_name = "co2_idINS_pdvE_MetAMP"
)
```
...
gg <- bd_read("distrev") 
gg[[3]] <- gg[[3]] + labs(caption="*Source* : MOBPRO, EMP 2019, C200, OSM, GTFS,  MEAPS. *version 2 (4/24)*<br>Chacun des points représente un IRIS. Les IRIS carrés sont à Marseille, les losanges à Aix-en-Provence,<br>les ronds partout ailleurs.<br>")
gg
```
...
download_this(
  ...
  output_name = "co2_IRIS_ndv_MetAMP"
)
```
...
download_this(
  ...
  icon = "fa fa-download",
  class = "dbtn",
  button_label = "données CO2 versus pollution à l'IRIS",
  output_name = "co2_IRIS_pol_MetAMP"
)
```
