...
[^commerces-1]: Et donc sans information de fréquence.
...
[^commerces-2]: Merci à Théo Shayer, Ludovic Verre et Luc Garnier pour nous avoir partagé leur méthodologie.
...
margin_link( dd, "amenites_idINS_pdvR_MetAMP", "données accessibilité aux aménités au carreau, résidents")
```
:::
...
[^commerces-3]: Cette valeur se lit sur le tableau en prenant le coefficient du dernier décile, moins celui du premier (égal à 0 par référence) et en élevant $e$ à la puissance de cette différence.
...
A partir des données de distance (ou de temps de parcours), de la localisation des résidents et des localisations des aménités, on peut construire un indicateur dit de "densité inversée" par lequel on représente la population qu'on balaye (soit à 15 minutes de marche ou de transport en commun, soit à 30 minutes de voiture).
...
margin_link(data, "dnwld/co2_motif_commerce_MetAMP", "données CO2 pour le motif courses")
