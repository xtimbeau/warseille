...
-   `idINS` : code [INSPIRE](https://knowledge-base.inspire.ec.europa.eu/index_en) (résolution 200m, coordonnées y et x en projection EPSG:3035). Peut être en format contracté où la résolution est enlevée, les lettres N et E sont omises et les coordonnées sont divisées par 200, ce qui permet de coder le `idINS` sur un entier). Nous avons omis dans le code INSPIRE les premiers caractères qui décrivent le système de coordonnées (`FR_CRS3035)` ainsi que le `m` de la résolution pour éviter de surcharger la chaîne de caractères. Le code est donc `r200NyyyyyyyExxxxxxx` où `yyyyyyy` et `xxxxxxx` sont les coordonnées dans la projection EPSG:3035 LAEA en mètres. Ce codage n'est pas très efficient, nous utilisons (normalement ce n'est plus présent dans les fichiers téléchargeables) un codage dérivé qui s'écrit `xxxxxxx/200*100000 +``yyyyyyy/200` qui est un entier à 10 chiffres bien plus petit que la chaîne de caractère standard.
...
```{r}
download_link(
  link = "dnwld/temps_accessibilite_idINS_MetAM.csv",
  icon = "fa fa-download",
  class = "dbtnlg",
...
```{r}
download_link(
  link = "dnwld/access_revenu_metAM.csv",
  class  = "dbtnlg",
  icon = "fa fa-download",
...
-   `sudair` : [ICAIR365](https://www.atmosud.org/article/icair-lindice-cumule-de-lair), qui cumule l'effet de 4 polluants (NO~2~, O~3~, PM2.5, PM10) sur une année (ici l'année 2022), construit par [AtmoSud](https://www.atmosud.org/), projeté au carreau 200m et agrégé à l'IRIS, pondéré par la population au carreau 200m.
...
-   `sa` : surface ajustée (voir document) d'aménités accessibles en 15 minutes à pied ou en transport en commun, calculé pour chaque type d'aménité
...
-   `se` : surface équivalente (voir document) ou indicateur de proimité des aménités accessibles en 15 minutes à pied ou en transport en commun, calculé pour chaque type d'aménité
...
-   `r` : résidu de la régression log linéaire (voir document)
