...
tbl <- res |> 
  ...
  cols_align("left", 1) |> 
  cols_align("center", -1) |> 
  tab_source_note(md("Le modèle estimé est pour chque zone où le nombre d'individus sondés est supérieur à 10. L'équation du modèle est $log(d) = a log(rev_{uc}) + b p_{enf} + c p_{act} + d_i \\mathcal{A}_i + e + \\varepsilon$ où les $i$ sont les quantiles pondérés par la population de l'indicateur de proximité (le premier quantile vaut 1 pour les 10% de la population qui ont la proximité la plus faible). Le premier décile est la valeur de référenc et n'est pas estimé"))
...
## Projection des distances et estiation des émissions de CO~2~ spatialisées
