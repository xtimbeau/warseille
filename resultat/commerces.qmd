...
ggg <- ggplot(gdata)+
  ...
  theme_ofce(
    ...
    legend.title = element_text(vjust = 0.5, hjust = 0.5, margin = margin(r = 12))
  ) +
  xlab("Déciles de niveau de vie")+
  ylab("Déciles d'excès ou déficit de proximité") +
  scale_color_manual(values = c(alim = "purple4", sortie = "orange4", sante = "seagreen", comm = "yellow3"),
                     labels = c("Commerces alimentaires", "Santé humaine", "Commerces non alimentaires", "Sorties")) +
  guides(color = "none") +
  ofce_caption(
...
[^3]: Cette valeur se lit sur le tableau en prenant le coefficient du dernier décile, moins celui du premier (égal à 0 par référence) et en élevant $e$ à la puissance de cette différence.
...
## Lien entre les émissions et la densité
...
```{r}
#| label: fig-dens_c
#| fig-cap: Emissions CO~2~ versus densité (iris)

prix_iris <- bd_read("km_iris") |> 
  select(iris = IRIS, prix, shape, dens.alt=dens, co2_p = co2_i)
...
(base <- ggplot(co2_iris) +
    aes(x=dens, y=co2_pi_c, fill = prix, weights=ind) +
    ...
    scale_y_continuous("tonnes de CO~2~ par an par actif motif commerce", labels = scales::label_number(big.mark = " ")) +
    geom_smooth(col="gold3", fill = "gold", aes(weight = ind), alpha=0.1) +
    theme_ofce(base_size = 10, legend.position = "bottom", marquee = TRUE )+
    theme(plot.margin = margin(),
          legend.key.width = unit(24, "pt"),
          legend.title = element_text(hjust = 1),
          legend.spacing = unit(48, "pt"),
...
ggplot(comp) +
  ...
  geom_hex(aes(weight = ind), binwidth = c(.025, .025)) +
  coord_equal(xlim = c(0,1.5), ylim=c(0,.75)) +
  scale_fill_gradient(
    name = "Individus",
    low = "gray95", high = "steelblue", transform = "sqrt",
    breaks = c(1, 1000, 10000, 25000)) + 
  xlab("Emissions par individu pour le motif professionnel") +
  ylab("Emissions par individu pour le motif commerces") +
  theme_ofce(
