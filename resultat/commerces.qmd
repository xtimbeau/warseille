...
memc2 <- ggplot() +
  ...
  geom_sf(data=coms, color = "black", linewidth = 0.2, fill = "transparent") +
...
vqh |> 
  ...
  coord_equal() +
  scale_x_log10(name = "surface équivalente des commerces alimentaires", limits = c(1, 1000)) +
  scale_y_log10(name = "surface équivalente des autres commerces", limits = c(1, 1000)) +
...
vqh |> 
  ...
  coord_equal() +
  scale_x_log10(name = "surface équivalente des commerces alimentaires", limits = c(1, 1000)) +
  scale_y_log10(name = "surface équivalente des sorties (bars, restaurants, cinéma)", limits = c(1, 1000)) +
...
vqh |> 
  ...
  scale_x_log10(name = "densité", limits = c(1, 1000))+
  scale_y_log10(name = "surface équivalente, alimentation", limits = c(1, 1000))+
  coord_equal() +
...
vqh |> 
  ...
  scale_x_log10(name = "densité", limits = c(1, 1000))+
  scale_y_log10(name = "surface équivalente, autres commerces", limits = c(1, 1000))+
  coord_equal() +
...
vqh |> 
  ...
  scale_x_log10(name = "densité", c(1, 1000))+
  scale_y_log10(name = "surface équivalente, autres commerces", limits = c(1, 1000))+
  coord_equal() +
...
labels <- set_names(c("Alimentaire", "Commerces non alimentaires", "Bars, restaurants, cinémas", "Santé humaine", "Synthétique"),  c("se_alim","se_comm","se_sortie","se_sante", "se_agg"))
lien_se_si |>
  filter(m == 1) |> 
  select(t, r2, c, se ) |> 
  mutate(t = labels[t],
  ...
  tab_source_note(md("Le modèle estimé est $log(\\mathcal(A)_c) = c \\times log(dens) + \\varepsilon$ où chaque carreau est une observation et la densité est le nombre d'habitants dans un rayon de 15 minutes à pied.") )
```
...
ggplot() +
  bd_read("decor_carte") +
  geom_sf( data = data, aes(fill = r), linewidth = 0.1, col = "white") +
  scale_fill_paletteer_c(palette = "ggthemes::Red-Green Diverging", trans = pseudo_log_trans())
```
## Non alimentaire
...
::: {.column-margin}
```{r}
data <- lien_se_si |> filter( m == 1) |> select(t, residus) |> unnest(residus) |> mutate(idINS = r3035::expand_idINS(idINS))
...
labels <- set_names(c("Alimentaire", "Commerces non alimentaires", "Bars, restaurants, cinémas", "Santé humaine", 
...
ggplot(data_emc2 |> filter(!is.na(q), n>10))+
  ...
  xlab("décile de surface équivalente ou de densité") +
...
tbl <- res |> 
  ...
  gt() |>
  fmt_markdown(c(rev_uc, enfants, actoccup, c, se, d1,d2,d3,d4,d5,d6,d7,d8,d9,d10)) |> 
  fmt_percent(c(r.squared, adj.r.squared), decimals = 1) |> 
  cols_label(r.squared = md("$R^2$"), adj.r.squared = md("$\\bar{R}^2$"), c = "constante", mod = "") |> 
...
ggplot() +
  ...
  geom_sf(
    data= proj,
    mapping= aes(fill = wqdist), col=NA) + 
  scale_fill_paletteer_d("fishualize::Bodianus_rufus", direction=1, 
                         label = labels_co2, name = NULL)+
  theme_ofce_void() +
  # coord_sf(xlim = c(bb$xmin, bb$xmax), ylim = c(bb$ymin, bb$ymax), crs = st_crs(3035)) +
  theme(legend.position = "bottom", 
        legend.title = element_text(size = rel(.7)),
        legend.key.height = unit(6, "pt"),
        legend.margin = margin(b=6)) +
  ofce_caption(source = "C200, Fichiers Foncier, EMC^2^. Calculs des auteurs, version 1.0")
