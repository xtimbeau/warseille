...
base <- ggplot(co2_iris) +
  ...
  scale_y_continuous("tCO2, motif commerce (moyenne de l'IRIS)", labels = scales::label_number(big.mark = " ")) +
...
base <- ggplot(co2_iris) +
  ...
  scale_x_continuous("Individus/ha", 
                     ...
                     limits = c(0,400)) + 
  scale_y_continuous("tCO2, motif commerce (moyenne de l'IRIS)", 
                     labels = scales::label_number(big.mark = " ")) +
  geom_smooth(col="gold3", fill = "gold", aes(weight = ind), alpha=0.1) +
  theme_ofce(base_size = 10, legend.position = "bottom", marquee = TRUE )+
  theme(plot.margin = margin(),
        legend.title = element_text(hjust = 1),
        axis.title = marquee::element_marquee(margin = margin(t=6)),
        axis.title.y = marquee::element_marquee(margin = margin(r=6)))
