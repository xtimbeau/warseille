...
Formellement, l'indicateur de proxiumité des aménités $\mathcal{A}$ est construit comme :
$$
...
ggplot() +
  bd_read("decor_carte") +
  geom_sf(
    data= data |> filter(type == "alim") |> drop_na(),
    mapping= aes(fill = wqse), col=NA) + 
  scale_fill_paletteer_d("PNWColors::Sunset2", direction=-1, 
                         name = "Accessibilité Alimentaire : ", label = c("très faible", "", "", "", "très élevé"))+
  theme_ofce_void() +
  ...
  note ="L'indicateur d'accessibilité aux aménités combine les surfaces commerciales ou de services dans un rayon de 15 minutes de marche ou de transport en commun avec une mesure de la diversité des commerces ou services accessible. Il est décliné selon 4 grandes catégories (ici commerces alimentaires)."))
```
...
ggplot() +
  ...
  geom_sf(
    ...
    mapping= aes(fill = wqse), col=NA) + 
  scale_fill_paletteer_c("PNWColors::Sunset2", direction=-1)+
  theme_ofce_void() +
