...
margin_link(td_access, "temps_accessibilite_idINS_MetAMP", "données temps d'accessibilité au carreau")
...
margin_link(dd, "access_par_com_MetAMP", "données temps d'accès par commune")
```
:::
...
gg_cmd <- list(
  ...
  theme(legend.text = element_text(margin=margin(t=2, l = 4, unit="pt"))))
```
...
margin_link( deciles, "deciles_MetAMP", "données répartition des déciles")
```
:::
...
margin_link(d_access,  "access_revenu_MetAMP", "données accessibilité et revenu au carreau")
```
:::
