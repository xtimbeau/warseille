...
-   Pour les **transports en commun**, les GTFS répertoriés sur [Open Data - lepilote](https://www.lepilote.com/fr/open-data/83), le moteur employé est R^5^ e Conveyal. On a retenu soit la médiane, soit le quantile 5% sur une plage horaire d'une heure (de 8h à 9h, le matin, un jour de semaine hors période de congé, en l’occurrence le 14/06/2023). Ceci permet de tenir compte des fréquences et des temps de correspondance (en fonction des horaires diffusés pour ce jour là). Le quantile 5% représente la possibilité de choisir son horaire de départ entre 8h et 9h pour minimiser son temps de trajet.
...
pal <- grDevices::colorRamp(c("seagreen4", "springgreen1", "hotpink", "deeppink3"), bias =1)( (0:60)/60)
...
ggplot(
  access_par_com |> 
    filter(mode %in% c("car_dgr", "transit5", "bike_tblr", "walk_tblr"))) +
