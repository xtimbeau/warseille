...
pplots <- ofce::source_data("commerces/proxplots.R", force=TRUE)
...
tabsetize2(pplots, girafy=FALSE, asp=1)
...
data <- bd_read("vqh") |>
  ...
  select(temps, type, idINS, ind, se, div, n, n_e, s, sb, sa) 
...
lplots <- ofce::source_data("commerces/lorplots.R", force=TRUE)
...
```{r, include=FALSE}
rplots <- ofce::source_data("commerces/resplots.R", force=TRUE)
```

```{r}
#| label: fig-regsise
#| fig-cap: Déficit ou excès de proximité par rapport à la densité de résidents
#| results: asis

tabsetize(rplots, asp=1, girafy=FALSE)
...
download_margin(data, "residus_prox-dens_MetAMP", "données résidus modèles proximité/densité (log)")
```
...
data <- bd_read("lien_se_si") |>
  ...
  left_join(bd_read("vqh") |> filter(temps == "15m tc/w") |> select(type, idINS = fromidINS, se ),
            by = c("idINS", "type")) |> 
  group_by(type) |> 
  mutate(
