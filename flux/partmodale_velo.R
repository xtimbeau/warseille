...
coms <- c200ze |> filter(scot) |> distinct(com) |> pull(com)
...
pmr |> 
  group_by(densite) |> 
  summarize(
    dlogco2 =  co2[s=="alt"]/co2[s=="ref"]-1,
    across(c(co2, co2_pi, car, bike, walk, transit), ~.x[s=="alt"]-.x[s=="ref"]) ) |> 
