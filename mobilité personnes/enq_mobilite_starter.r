...
# zip::unzip(zipfile = "~/files/DVFdata/sources/mobilite/donnees_individuelles_anonymisees_emp2019.zip",
...
deploc <- deploc |> 
  ...
  left_join(select(tcm_men, ident_men, pond_menC, decile_rev, quartile_rev, contains(c("RES", "res"))), 
...
deploc <- deploc |> 
  mutate(across(.cols = MMOY1S:MMOY4S, .fns = cut, breaks = breaks_transport, 
...
deploc <- deploc |> filter(!is.na(mode1))
