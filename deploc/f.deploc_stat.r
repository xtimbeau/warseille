deploc_densite <- function(data, reg="all", K=512, label_reg = NULL,
  ...
  data <- data |> 
    ...
    mutate(motifbin = motif_principal,
           motifbin = factor(motifbin,
                             c("travail","etudes", "courses", "autres"))) 
  ...
  calc <- furrr::future_map_dfr(1:K, ~{
    ...
    km_ad <- km_dens |> 
      ...
      transmute(densite, var="kmad", across(c(travail, autres, courses, etudes, total), ~.x/adultes))
    ...
    trajets_ad <- km_dens |> 
      filter(var == "trajets") |> 
      left_join(adultes_dens, by="densite") |> 
      transmute(densite, var="trad", across(c(travail, autres, courses, etudes, total), ~.x/adultes)) 
    ...
    distrj_ad <- left_join(km_ad, trajets_ad, by="densite", suffix = c(".km", ".trj")) |> 
      ...
      select(densite, travail, autres, courses, etudes, total) |> 
      mutate(var = "dtrjad")
  ...
  calc_r <- calc |> 
    group_by(var, s) |> 
    mutate(across(
      c(travail, autres, courses, etudes, total), ~.x/.x[densite==d1])) |> 
    ungroup() |> 
...
deploc_densite_2 <- function(data, reg="all", K=512, label_reg = NULL,
  ...
  calc <- furrr::future_map_dfr(1:K, ~{
    ...
    ind_aggr <- data.l |> 
      ...
      pivot_longer(cols = -DENSITECOM_RES) |> 
